...
def input_dictionary_gen(logic_combination: list, found_inputs: list, found_logic_gates: list) -> dict:
    ...
    found_inputs = ' '.join(found_inputs).lower().split(' ')
    for i in range(len(found_inputs)):
        wrong_tries = 0
        if len(found_inputs[i]) > 1:
            input_dictionary[found_inputs[i]] = ''
            break
        while True:
            if found_inputs[i] in input_dictionary:
                break
            tmpList = list(input(f"Enter the binary set for the '{found_inputs[i]}' input:\n"))
            tmpList = [int(char) for char in tmpList if char.isdigit()]
            if len(tmpList) != 4 or any(char != 1 and char != 0 for char in tmpList):
                print("Not valid binary set, retry...")
                wrong_tries += 1
                time.sleep(0.3)
            else:
                input_dictionary[found_inputs[i]] = tmpList
                break
            if wrong_tries > 5:
...
def output_dictionary_gen(input_dictionary: dict, logic_combination: list, inputs: list, logic_gates: list) -> dict:
    # Saves all the indexes of the inputs, to then check if there are 'NOT's
    input_indexes     = [logic_combination.index(input) for input in logic_combination if input in inputs]
    gate_indexes      = [logic_combination.index(gate) for gate in logic_combination if gate in logic_gates]
    output_dictionary = dict()
    calculated_inputs = dict()
    is_not            = dict()

    # Update the inputs dictionary, by calling the NOTs function for all the found NOT inputs,
    # To make easier calculations in the output dictionary generation algorithm
    for i in input_indexes:
        is_before = -1
        gate_before_input = logic_combination[i - 1]
        gate_after_input  = logic_combination[i + 1]

        logic_combination.append(None)
        if gate_before_input == 'NOT':
            is_before = 2
        elif gate_after_input == 'NOT':
            is_before = 0

        if is_before != -1 and len(logic_combination[i]) > 1:
            is_not[logic_combination[i] + str(i)] = 'NOT'
        if is_before != -1 and len(logic_combination[i]) < 2:
            input_dictionary[logic_combination[i].lower()] = getattr(LogicGates, 'NOT')(input_dictionary[logic_combination[i].lower()])
            logic_combination[i + 1 - is_before] = None

        # Saves the information in a dictionary, to then check if the input should go through NOT,
        # This is done because input longer than 1 need every element in them to be calculated

    print(is_not)
    for i in gate_indexes:
        input_before_gate = logic_combination[i - 1].lower()
        input_after_gate  = logic_combination[i + 1].lower()
        gate = logic_combination[i]



        #input_dictionary[] = (getattr(LogicGates, gate)(input_dictionary[input_before_gate], input_dictionary[input_after_gate]))
        '''
        if input_before_gate

        elif input_before_gate not in is_not and input_before_gate not in calculated_inputs:

        if input_after_gate in is_not and input_after_gate not in calculated_inputs:

        elif input_after_gate not in is_not and input_before_gate not in calculated_inputs:
        '''
...
#def table_write(dir_path: str, file_path: str, output_dictionary: dict):
...
if __name__ == "__main__":
    sys.exit("This package is not runnable as a standalone")
